 ...
 * @author tolua
 */
@ManagedBean(name="newCardStopReason")
@RequestScoped
public class NewCardStopReason implements Serializable{

private Integer errorfieldcount;

private String newCardSRDescript;
public String getNewCardSRDescript() {
return newCardSRDescript;
}
public void setNewCardSRDescript(String newCardSRDescript) {
this.newCardSRDescript = newCardSRDescript;
}
...
this.newAllinitialAudit = newAllinitialAudit;
   ...
   private String newCardSRIDselection;
   private Map<String,String> newCardSRIDoptions = new LinkedHashMap<>();
...
String ymenutype =(String)session.getAttribute("MenuType");
        ...
        newCardSRIDoptions = new LinkedHashMap<>();
        for (int i = 1; i < 201; i++){
            newCardSRIDoptions.put(String.valueOf(i),String.valueOf(i));
        }

        
   }

//********************************************CardSRID declartions

   public String getNewCardSRIDselection() {
   	return newCardSRIDselection;
   }

   public void setNewCardSRIDselection(String newCardSRIDselection) {
   	this.newCardSRIDselection = newCardSRIDselection;
   }

   public Map<String, String> getNewCardSRIDoptions() {
   	return newCardSRIDoptions;
   }

   public void setNewCardSRIDoptions(Map<String, String> newCardSRIDoptions) {
   	this.newCardSRIDoptions = newCardSRIDoptions;
   }
...
Connection connection=null;
        try {
 	 	...
 	 	ps=connection.prepareStatement("select * from cardstoptemp WHERE CardSTReasonID = " + "'" + newCardSRIDselection + "'");
...
int ckdigits = 0;
        try {
            ...
            DBConnection obj_DB_connection=new DBConnection();
            connection=obj_DB_connection.get_connection();
 	 	ps=connection.prepareStatement("select * from cardstop WHERE CardSTReasonID = " + "'" + newCardSRIDselection + "'");
 	 	ResultSet rs=ps.executeQuery();
                rs.beforeFirst();
 	 	while(rs.next()){
...
//--------------------------updating database
        ...
        String sql = "insert into cardstoptemp(CardSTReasonID, CardSTReasonDES, RecordStatus,Inputter,InputterRec,Authoriser,AuthoriserRec,updatetype) values (?,?,?,?, ?, ?, ?, ?)";
	statement = connection.prepareStatement(sql);
			
			statement.setString(1, newCardSRIDselection);
			statement.setString(2, newCardSRDescript);
			...
			statement.execute();
                        ...
                        String protocolresult = GetProtocolUpdate.UpdateRecords(yuser, "CARD.STOP.REASON", newCardSRIDselection, "I", null, AuditDateRecord,null,ytransit);
                        
      
       FacesMessage SuccessMessage = new FacesMessage(FacesMessage.SEVERITY_INFO, "Transaction Completed", "Record ID: " + newCardSRIDselection);
